...
function BoxMesa({ searchTerm }) {
  ...
  useEffect(() => {
    const fetchMesa = async () => {
      try {
        const response = await axios.get("https://iycy90ubq9.execute-api.us-east-1.amazonaws.com/build/api/v1/mesa");
  ...
  const handleRemoveMesa = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete("https://iycy90ubq9.execute-api.us-east-1.amazonaws.com/build/api/v1/mesa", {
